...
![单Agent流程v4_详细图](./asset/单Agent流程v4_详细图.jpg)
...
对于任意指令，由任务管理Agent自主决策规划任务流程，并自主决策从候选Agent池中选择需要的Agent并为其分配任务。同时协作过程中的任务流程调整与协作调整均由任务管理Agent完成。

（在MetaGPT中任务的更新与追加是通过LLM输出命令的形式。单独定义了一个命令集，作为prompt指导LLM遵循格式输出特定格式的命令）
...
允许人类操作员中途加入任务群组进行观察或参与协作（人类操作员需要和群组中任务管理Agent明确自己的协作内容，由任务管理Agent调整任务流程以在其中加入人类环节）。
...
- Task Group 任务群组（一个Team可以同时存在多个Task Group）：
  ...
  团队被分配的多个任务中，每一个任务会有自己的一个任务群组。任务群组由多个Agent组成。对每个Agent而言，可能同时参与多个任务群组。一个任务群组至专注于一个任务（一个完整任务流程），当任务完成时，该群组解散。
...
- Agent核心工作流构建
  ...
  Agent工具调用路由，任务分配与协作通信。

- LLM System评估系统搭建

  构建Agent轨迹跟踪与监控系统，维护一致性测试数据集，完成评估测试
...
- 接入业务的人类交互前端
  ...
  人类操作端（以实例化一个特殊Agent的形式，使用相同的通信方式完成与Agent的交互）
