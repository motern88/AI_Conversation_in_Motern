...
> > 其中：
> >
> > - $\frac{1}{Z}$ 是归一化常数，用于确保整个概率密度函数的积分值为 1，满足概率的基本性质。
> >
> >   在这个高斯分布的表达式中，$Z$ 的具体形式是：
> >   $$
> >   Z=\sqrt{(2π)^d σ^{2d}}
> >   $$
> >   其中 $d$ 是 $x$ 的维度。例如，在一维情况下，$d=1$，$Z=\sqrt{2π σ^2}$；在二维情况下，$d=2$，$Z=\sqrt{(2π)^2σ^4}=2πσ^2$。这个归一化常数的作用是使得整个高斯分布的积分（在所有可能的 $x$ 值上）等于 1。
> >
> > - $∥x−μ∥^2 $ 是 $x$ 与均值 $μ$ 之间的欧几里得距离的平方。
> >
> >   它表示从高斯分布的中心点 $μ$ 到点 $x$ 的直线距离的平方。例如，在二维空间中：
> >   $$
> >   ∥x−μ∥ ^ 2=(x_1 − μ_1)^2 + (x_2 − μ_2)^2
> >   $$
> >
> > - $−\frac{1}{2σ^2}$ 是一个负的系数，它使得整个指数函数的值随着 $∥x−μ∥^2$ 的增大而减小。
> >
> >   这个系数的 $\frac{1}{2}$ 是为了简化积分计算，使得在计算高斯分布的积分时，能够方便地得到归一化常数 $Z$。
> >
> > - 整个指数函数 $exp(−\frac{1}{2σ^2}∥x−μ∥^2)$ 的值随着 $x$ 与 $μ$ 之间距离的增大而快速减小。
> >
> >   当 $x$ 接近 $μ$ 时，指数函数的值接近 1；当 $x$ 越远离 $μ$ 时，指数函数的值迅速趋近于 0。这表示在高斯分布中，距离中心点 $μ$ 较近的点有更高的概率密度，而距离中心点较远的点有较低的概率密度。
> >
> > 从几何上来看，这个高斯分布表示的是一个以 $μ$ 为中心，扩散程度由 $σ^2$ 决定的各向同性分布。在二维情况下，其等高线是圆形，圆心在 $μ$，圆的半径越大，表示距离中心越远的概率密度越小；在三维情况下，其等高线是球形，球心在 $μ$，球的半径越大，表示距离中心越远的概率密度越小。
...
$$
> **步骤 1：简化第一项**  
> 利用 $\bar\alpha_t = \bar\alpha_{t-1} \alpha_t$，有 $\sqrt{\bar\alpha_t} = \sqrt{\bar\alpha_{t-1}} \sqrt{\alpha_t}$，因此：
> $$
> \frac{1}{\sqrt{\bar\alpha_t}} = \frac{1}{\sqrt{\bar\alpha_{t-1}} \sqrt{\alpha_t}}
> $$
> 代入第一项：
> $$
...
> 表达式更新为：
> $$
> \tilde\mu_t(x_t, \epsilon) = \frac{\beta_t}{(1 - \bar\alpha_t) \sqrt{\alpha_t}} \left(x_t - \sqrt{1-\bar{\alpha}_t} \epsilon \right) + \frac{\sqrt{\alpha_t}(1 - \bar\alpha_{t-1})}{1 - \bar\alpha_t} x_t
> $$
>
> **步骤 2：提取公因子 $\frac{1}{1 - \bar\alpha_t}$**  
> $$
> \tilde\mu_t(x_t, \epsilon) = \frac{1}{1 - \bar\alpha_t} \left[ \frac{\beta_t}{\sqrt{\alpha_t}} \left(x_t - \sqrt{1-\bar{\alpha}_t} \epsilon \right) + \sqrt{\alpha_t} (1 - \bar\alpha_{t-1}) x_t \right]
> $$
>
> **步骤 3：展开括号内表达式**  
> $$
> = \frac{1}{1 - \bar\alpha_t} \left[ \frac{\beta_t}{\sqrt{\alpha_t}} x_t - \frac{\beta_t}{\sqrt{\alpha_t}} \sqrt{1-\bar{\alpha}_t} \epsilon + \sqrt{\alpha_t} (1 - \bar\alpha_{t-1}) x_t \right]
> $$
> 合并 $x_t$ 的系数：
> $$
> \text{系数} = \frac{\beta_t}{\sqrt{\alpha_t}} + \sqrt{\alpha_t} (1 - \bar\alpha_{t-1})
> $$
> 代入 $\beta_t = 1 - \alpha_t$：
> $$
> = \frac{1 - \alpha_t}{\sqrt{\alpha_t}} + \sqrt{\alpha_t} (1 - \bar\alpha_{t-1}) = \frac{1}{\sqrt{\alpha_t}} - \sqrt{\alpha_t} + \sqrt{\alpha_t} - \bar\alpha_{t-1} \sqrt{\alpha_t} = \frac{1}{\sqrt{\alpha_t}} - \bar\alpha_{t-1} \sqrt{\alpha_t}
> $$
> 利用 $\bar\alpha_{t-1} \alpha_t = \bar\alpha_t$：
> $$
> = \frac{1 - \bar\alpha_t}{\sqrt{\alpha_t}}
> $$
> 括号内表达式简化为：
> $$
> \frac{1 - \bar\alpha_t}{\sqrt{\alpha_t}} x_t - \frac{\beta_t}{\sqrt{\alpha_t}} \sqrt{1-\bar{\alpha}_t} \epsilon
> $$
>
> **步骤 4：代入整体表达式**  
> $$
> \tilde\mu_t(x_t, \epsilon) = \frac{1}{1 - \bar\alpha_t} \left[ \frac{1 - \bar\alpha_t}{\sqrt{\alpha_t}} x_t - \frac{\beta_t}{\sqrt{\alpha_t}} \sqrt{1-\bar{\alpha}_t} \epsilon \right]
> $$
> 简化：
> $$
> = \frac{1}{1 - \bar\alpha_t} \cdot \frac{1 - \bar\alpha_t}{\sqrt{\alpha_t}} x_t - \frac{1}{1 - \bar\alpha_t} \cdot \frac{\beta_t}{\sqrt{\alpha_t}} \sqrt{1-\bar{\alpha}_t} \epsilon = \frac{1}{\sqrt{\alpha_t}} x_t - \frac{\beta_t}{\sqrt{\alpha_t} \sqrt{1 - \bar\alpha_t}} \epsilon
> $$
> 提取公因子 $\frac{1}{\sqrt{\alpha_t}}$：
> $$
