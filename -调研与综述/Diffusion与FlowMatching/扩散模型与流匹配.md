...
因此，最终我们可以推导得到：
$$
q(x_{t-1} \mid x_t, x_0) = \mathcal{N}\left(x_{t-1}; \tilde\mu_{t-1}(x_t, x_0), \tilde\beta_t \mathbf{I} \right)
$$
...
代入 $x_0$ 得到：
$$
\tilde\mu_{t-1}(x_t, \epsilon) = \frac{\sqrt{\bar\alpha_{t-1}} \beta_t}{1 - \bar\alpha_t} \cdot \frac{1}{\sqrt{\bar{\alpha}_t}} \left(x_t - \sqrt{1-\bar{\alpha}_t} \epsilon \right) + \frac{\sqrt{\alpha_t}(1 - \bar\alpha_{t-1})}{1 - \bar\alpha_t} x_t
$$




*TODO:教程中上式可以化简为下式，然而GPT认为不能化简为下式。*
$$
\tilde\mu_{t-1} = 
...
*TODO：教程中是$\tilde\mu_{t}$，而上述GPT推导是$\tilde\mu_{t-1}$，孰对孰错？*
